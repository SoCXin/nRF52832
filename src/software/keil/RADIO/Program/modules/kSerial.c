...
/* Private macro ---------------------------------------------------------------------------*/
/* Private variables -----------------------------------------------------------------------*/
static NRF_UART_Type *kSerialUart = NULL;
...
{
  do {
    kSerialUart->TXD = *sendData++;
    while (UART_EVENTS_TXDRDY(kSerialUart) != SET);
    UART_EVENTS_TXDRDY(kSerialUart) = RESET;
  } while (--lens);
}
...
{
  while (UART_EVENTS_RXDRDY(kSerialUart) != SET);   // Wait for RXD data to be received
  return ((uint8_t)kSerialUart->RXD);
}
